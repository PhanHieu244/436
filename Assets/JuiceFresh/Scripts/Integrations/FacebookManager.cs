...
public class FacebookManager : MonoBehaviour {
    ...
    public static List<FriendData> FriendsPlayFab = new List<FriendData>();
    ...
    void OnEnable() {
#if PLAYFAB
        PlayFabManager.OnLoginEvent += GetUserName;
...
#if PLAYFAB
            PlayFabManager.facebookUserID = AccessToken.CurrentAccessToken.UserId;
            PlayFabManager.THIS.LoginWithFB(AccessToken.CurrentAccessToken.TokenString);
#endif
        }
        else {
...
#if PLAYFAB
            PlayFabManager.THIS.UpdateName(fbname);
#endif
...
#if PLAYFAB
            PlayFabManager.PlayerPictureLoaded();
                ...
                if (dataArray.Count > 0) {
                    for (int i = 0; i < dataArray.Count; i++) {
                        ...
                        foreach (KeyValuePair<string, object> entry in firstGroup) {
                            if (entry.Key == "id")
                                friendID = "" + entry.Value;
                            if (entry.Key == "picture") {
                                var pictures = (Dictionary<string, object>)entry.Value;
                                foreach (KeyValuePair<string, object> pictureUrl in pictures) {
                                    ...
                                    foreach (KeyValuePair<string, object> pictureUrl1 in pictures1) {
                                        if (pictureUrl1.Key == "url") {
                                            FriendData friend = FriendsPlayFab.Find(delegate (FriendData bk) {
